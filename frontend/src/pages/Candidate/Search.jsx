  ...
  const applyFilter = () => {
    ...
    if (minSalary !== "") {
      filteredJobs = filteredJobs.filter(
        (job) => parseInt(job.minSalary) >= parseInt(minSalary)
      );
    }

    if (maxSalary !== "") {
      filteredJobs = filteredJobs.filter(
        (job) => parseInt(job.minSalary) <= parseInt(maxSalary)
      );
    }
            ...
            (filteredJobs || searchJobs).map((job, index) => (
              <Link to={`/jobdetails/${job._id}`} key={index}>
                <div className="flex flex-col  items-center my-7">
                  <div
                    className="bg-zinc-50  flex flex-col items-center md:flex-row justify-center md:mr-72  p-2 overflow-hidden rounded-lg shadow-lg border-2 border-zinc-100 "
